...
Top `r myn` types of weather events that kill or injure most people, and their overlap:
```{r humanloss, cache=TRUE}
...
The plots include a set of bars for only the last `r myy` years of data because it might be of interest to see how their heights compare: they make it clear that the casualty counts are disproportionately large over the past `r myy` years. This may be a normal consequence of having more people at risk from normal population increase.

Also, paired red and blue bars -- the case for most event types -- make it easy to see that the ranking of most catastrophic events has not changed much over time, though this may be the normal result of having the latest `r myy` years' counts carry so much weight in the totals of the worst events. This is especially obvious in the injuries plot.
