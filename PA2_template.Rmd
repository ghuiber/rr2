...
For example, there are `r length(unique(data$EVTYPE))` unique event values in the data set, but if EVTYPE is forced to all lowercase there remain `r length(unique(tolower(data$EVTYPE)))` of them. This would be an easy fix, but there are also plurals, idiosyncratic spaces (e.g. "W IND"), typos, and an amazing variety of ways to spell "thunderstorm". Some of these things can be fixed with regular-expression matching, as below:
...
Excluding summaries, I started with `r foo` unique event types, and after tidying up a bit as shown above, I ended up with `r bar` unique event types. I'm not claiming that I caught every source of event type confusion, but surely this is an improvement over the original set.
...
Paired red and blue bars -- the case for most event types -- make it easy to see how the ranking of most catastrophic events has changed over time. Missing blue bars correspond to event types that have been in the top `r myn` most harmful in years before the last `r myy`. Missing red bars correspond to event types that are among the top `r myn` top harmful in the most recent `r myy` years, but didn't use to be before. 

The common set of event types between those who killed the most people in the last `r myy` years and those who injured the most people over the same period is listed below:
...
```{r econdmg, fig.width=12}
pe1
```

In the same way that more people are in harm's way in more recent years as a natural consequence of population growth, more assets are also in harm's way in more recent years as a consequence of both actual wealth growth and presumably inflation. So, again it makes sense to split the data between the most recent `r myy` years and the rest, and plot it that way. Because the bars had to be ordered somehow, they're ordered by magnitude of the damage over the most recent `r myy` years. This is not the same as in the previous years, and again missing bars -- either blue or red -- tell the story of chages over time in the top `r myn` ranking.

The types of events ranked above are responsible for `r round(100*getShareOfLoss(econdmg$EVTYPE,'econdmg',peoplesum),0)`% of all recorded economic damage in the StormData file. That share is `r round(100*getShareOfLoss(econdmg$EVTYPE,'econdmg',peoplenew),0)`% if measured over the last `r myy` years only. 

Tornadoes top the list again, but by a much narrower margin and they weren't always the worst thing: before the last `r myy` years, thunderstorm winds and flash floods both beat tornadoes at asset destruction.
